  ...
  if(!hamburger)return(
    <div className='w-100  font-bold text-lg text-white bg-transparent flex justify-between z-[50000] pointer-events-auto'>
      ...
      <div className='flex gap-8'>
        {!home && <Link href={"/#services"} className="my-auto cursor-pointer scroll-smooth">Services</Link>}
        {home && <button onClick={()=>{window.scrollTo({top:viewportHeight!, behavior:"smooth"})}} className="my-auto cursor-pointer">Services</button>}
        <Link href={"/careers/sales-executive"} className="my-auto cursor-pointer">Careers</Link>
        {!home && <Link href={"/#form"} className="bg-white text-blue-500 font-bold py-2 px-4 cursor-pointer">Get a Quote</Link>}
        {home && <button className="bg-[#EB9B2F] text-white rounded-full font-bold py-2 px-4 cursor-pointer" onClick={()=>{window.scrollTo({top:viewportHeight! + 100, behavior:'smooth'})}}>Get a Quote</button>}
      </div>
    </div>
  )
  else return(
      ...
      {open &&
        <div className='flex flex-col z-20 absolute top-0 w-full h-fit bg-white'>
          {home && <button onClick={()=>{
            ...
            setOpen(false)}} className="my-auto text-blue-500 font-bold py-2 px-4 w-fit">Services</button>}
          {!home && <Link href={"/#services"} className="my-auto text-blue-500 font-bold py-2 px-4 w-fit">Services</Link>}
          ...
          {home && <button onClick={()=>{
            ...
            setOpen(false)}} className=" text-blue-500 font-bold py-2 px-4 w-fit">Get a Quote</button>}
          {!home && <Link href={"/#form"} className="bg-[#EB9B2F] text-blue-500 font-bold py-2 px-4">Get a Quote</Link>}
